...
export default async function MovimientoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: tx }, { data: accounts }, { data: categories }] = await Promise.all([
    supabase
      .from("transactions")
      .select("id, type, amount, occurred_on, notes, account_id, to_account_id, category_id")
      .eq("id", id)
      .maybeSingle(),
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <EditarMovimientoForm transaction={tx} accounts={accounts ?? []} categories={categories ?? []} />
    </div>
  );
}
